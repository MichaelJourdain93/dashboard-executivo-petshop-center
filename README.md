...
- **The NPS visual was summing the scores instead of averaging them** — it displayed 9539 for detractors when the real average score is 2.97, inverting the chart's reading: the worst group appeared with the highest "score". **Already fixed**: the aggregation was switched from `sum` to `average`, and the panel now shows 2.97 / 7.49 / 9.49, matching the independent recalculation. The bug was invisible on screen — the numbers looked plausible — and only surfaced when the indicators were recomputed outside the tool.
...
- **O visual de NPS estava somando as notas em vez de tirar a média** — exibia 9539 para os detratores quando a nota média real é 2,97, invertendo a leitura do gráfico: o pior grupo aparecia com a "nota" mais alta. **Já corrigido**: a agregação foi trocada de `sum` para `average` e o painel agora exibe 2.97 / 7.49 / 9.49, batendo com o recálculo independente. O erro era invisível na tela — os números pareciam plausíveis — e só apareceu ao recalcular os indicadores fora da ferramenta.
